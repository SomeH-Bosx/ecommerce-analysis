...
- 修改文件：
  - `README.md`：按开源数据项目习惯重写
  - `dev-log.md`：本步日志

## [2026-09-06] 增加一键运行器 main.py

- 做了什么：用 `main.py` 串起清洗、SQL、RFM、经营日报四步；README 标题改为「一键生成电商经营 Dashboard 与 AI 经营日报」。
- 修改文件：
  - `main.py`：按顺序调用四个脚本，失败即停
  - `README.md`：改标题，从零运行改为 `python main.py`
  - `dev-log.md`：本步日志

## [2026-09-06] 收口：核对口径并写清复现步骤

- 做了什么：对照 DuckDB 复核 KPI / RFM / 品类 / 州数字，把 README 改成可从零复现的说明，并标出不入库文件。项目六阶段完成。
- 修改文件：
  - `README.md`：运行命令、核对表、不入库清单、阶段收口
  - `dev-log.md`：本步日志
